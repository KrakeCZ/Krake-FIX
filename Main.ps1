...
function Show-MainMenu {
    ...
    while ($true) {
        ...
        Write-Host "[10] Lehké Gaming Performance Tweaks" -ForegroundColor Red
        Write-Host "[11] Zákaz Telemetrie & Další služby" -ForegroundColor Red
        ...
        Write-Host "[18] 💾 Disk BOOST - NVMe/SSD  (Registry Nvme + FeatureM + Fsutil)" -ForegroundColor Cyan
        Write-Host "--------------------------------------------------"
        Write-Host "[b] 🚀 BackgroundProcessEliminator (Export na plochu)" -ForegroundColor Green
        Write-Host "--------------------------------------------------"
        ...
        switch ($choice) {
            ...
            '18' {
                if (-not (Invoke-ModuleEntryPoint -ModuleName 'TweakD')) {
                    if (Get-Command Invoke-TweakD -ErrorAction SilentlyContinue) {
                        Invoke-TweakD
                    }
                    else {
                        Write-Warning "TweakD.psm1 není načten."
                        Start-Sleep -Seconds 2
                    }
                }
            }
            'b' {
                $desktopPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), "BPE.bat")
                $bpeContent = @"
@echo off
...
"@
                try {
                    $bpeContent | Out-File -FilePath $desktopPath -Encoding ascii -Force
                    Write-Host "`n✅ Soubor BPE.bat byl vytvořen na ploše." -ForegroundColor Green
                }
                catch {
                    Write-Error "❌ Nepodařilo se vytvořit soubor na ploše: $($_.Exception.Message)"
                }
                Start-Sleep -Seconds 2
            }
            'Q' {
...
function Show-TweaksMenu {
    ...
    while ($true) {
        ...
        Write-Host ""
        Write-Host "[0] 🛡️  REGISTRY ONLY - Pouze registry (Bez AppX/Debloat)" -ForegroundColor Cyan
        Write-Host "    → Aplikuje pouze optimalizace registrů + fsutil optimalizace " -ForegroundColor Gray
        Write-Host "    → Nemaže žádné aplikace, úlohy, služby" -ForegroundColor Gray
        Write-Host ""
        ...
        Write-Host "    → XBOX stahni z webu MS, instaluje msStore, aktuálně zachován" -ForegroundColor Gray
        Write-Host "    ! pokud chceš odstranit i Xbox, uprav skript- přidej název package" -ForegroundColor Gray
        Write-Host ""
        Write-Host "[D] 💾 TWEAK D - NVMe/SSD Boost (Registry + fsutil)" -ForegroundColor Yellow
        Write-Host "    → Optimalizace pro NVMe disky (FeatureManagement + fsutil)" -ForegroundColor Gray
        Write-Host ""
        Write-Host "[V] 🖼️  Obnova starého Prohlížeče fotek" -ForegroundColor Cyan
        Write-Host "    → Aktivuje starý klasický Windows Photo Viewer" -ForegroundColor Gray
        Write-Host ""
        Write-Host "──────────────────────────────────────────────────────────────" -ForegroundColor DarkGray
        Write-Host "[R] 🔄 TWEAK R - Reset služeb do výchozího stavu" -ForegroundColor Magenta
        Write-Host "    → Obnovuje všechny služby (Automatic/Running)" -ForegroundColor Gray
        Write-Host "    → Používá SYSTEM oprávnění! " -ForegroundColor Gray
        Write-Host ""
        Write-Host "[T] 🔄 TWEAK T - Obnova naplánovaných úloh (Reset Task)" -ForegroundColor Magenta
        Write-Host "    → Povolí a spustí úlohy zakázané v TweakC-ULTRA(zvol i pro B)" -ForegroundColor Gray
        Write-Host ""
        ...
        switch ($choice.ToUpper()) {
            '0' {
                if (-not (Invoke-ModuleEntryPoint -ModuleName 'Tweak0')) {
                    if (Get-Command Invoke-Tweak0 -ErrorAction SilentlyContinue) {
                        Invoke-Tweak0
                    }
                    else {
                        Write-Warning "Tweak0.psm1 není načten."
                        Start-Sleep -Seconds 2
                    }
                }
            }
            ...
            'V' {
                if (-not (Invoke-ModuleEntryPoint -ModuleName 'TweakV')) {
                    if (Get-Command Invoke-TweakV -ErrorAction SilentlyContinue) {
                        Invoke-TweakV
                    }
                    else {
                        Write-Warning "TweakV.psm1 není načten."
                        Start-Sleep -Seconds 2
                    }
                }
            }
            ...
            'T' {
                if (-not (Invoke-ModuleEntryPoint -ModuleName 'TweakT')) {
                    if (Get-Command Invoke-TweakT -ErrorAction SilentlyContinue) {
                        Invoke-TweakT
                    }
                    else {
                        Write-Warning "TweakT.psm1 není načten."
                        Start-Sleep -Seconds 2
                    }
                }
            }
