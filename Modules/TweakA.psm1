# ═══════════════════════════════════════════════════════════════════════════
# Module: TweakA.psm1 - AGRESIVNÍ DEBLOAT (50 balíčků + HP)
# ═══════════════════════════════════════════════════════════════════════════
# REŽIM A: AGRESIVNÍ
# - 64 Microsoft balíčků
# - ~26 HP/OEM bloatware
# - ✅ ZACHOVÁ: Kalkulačka, Fotky, Kalendář, MsTeams,OneDrive,Telephone
# - ✅ ZACHOVÁ: Xbox App, MsStore, Xbox Identity,Notes,Copilot
# - ❌ BEZ Edge uninstall
# ═══════════════════════════════════════════════════════════════════════════
...
$script:TweakARegistryListCache = $null

function Convert-TweakARegistryPath {
    [CmdletBinding()]
    param(
...
$script:TweakAServiceList = @(
    [pscustomobject]@{ Name = "DevQueryBroker"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "DiagSvc"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "DiagTrack"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "Diagnosticshub.standardcollector.service"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "dmwappushservice"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "HPOMENHSAService"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "HPSysInfoHSAService"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "Fax"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "lfsvc"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "MapsBroker"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "TroubleshootingSvc"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "tzautoupdate"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "WdiServiceHost"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "WdiSystemHost"; Status = "Stopped"; StartType = "Disabled" },
    [pscustomobject]@{ Name = "wisvc"; Status = "Stopped"; StartType = "Disabled" }
)


function Get-TweakAServiceList {
    [CmdletBinding()]
    [OutputType([pscustomobject[]])]
    param()

    return $script:TweakAServiceList
}
function Invoke-TweakA {
    [CmdletBinding()]
    param()

    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Magenta
    Write-Host "  TWEAK A -  DEBLOAT (50 balíčků + HP)" -ForegroundColor Magenta
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Magenta
    Write-Host ""
    Write-Host "⚠️  VAROVÁNÍ: aplikace trva 1-5min !" -ForegroundColor Red
    Write-Host ""
    ...
    $regContent = Get-TweakARegContent
    ...
    # FÁZE 2.5: SLUŽBY 
    # ───────────────────────────────────────────────────────────────────────
    Write-Host "FÁZE 2.5: KONFIGURUJI SLUŽBY..." -ForegroundColor Yellow
    Write-Host ""

    $serviceList = Get-TweakAServiceList
    
    if ($serviceList.Count -gt 0) {
        Write-Host "  🔐 Spouštím s SYSTEM oprávněními (hromadná operace)..." -ForegroundColor Cyan
        
        $disableServicesBlock = {
            param($svcList)
            $count = 0
            
            # Helper function inside SYSTEM context
            function Set-ServiceConfig {
                param($Name, $Status, $Startup)
                
                # Resolve wildcards if any (simple check)
                $targets = @($Name)
                if ($Name -match '\*') {
                    $targets = @(Get-Service -Name $Name -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name)
                }
                
                foreach ($svcName in $targets) {
                    if (Get-Service -Name $svcName -ErrorAction SilentlyContinue) {
                        try {
                            # 1. Stop service if requested
                            ...
                            if (-not [string]::IsNullOrWhiteSpace($Startup)) {
                                # Use sc.exe for reliable startup type setting (including disabled)
                                $startMode = switch ($Startup) {
                                    'Automatic' { 'auto' }
                                    'Manual'    { 'demand' }
                                    'Disabled'  { 'disabled' }
                                    Default     { 'demand' }
                                }
                                $null = & sc.exe config $svcName start= $startMode 2>&1
                            }
                            
                            # 3. Start if requested
                            if ($Status -eq 'Running' -and $Startup -ne 'Disabled') {
            ...
            foreach ($svcItem in $svcList) {
                Set-ServiceConfig -Name $svcItem.Name -Status $svcItem.Status -Startup $svcItem.StartType
            }
            return $count
        }

        try {
            $disabledCount = Invoke-AsSystem -ScriptBlock $disableServicesBlock -ArgumentList (,$serviceList)
            Write-Host "  ✅ Upraveno: $disabledCount služeb (SYSTEM režim)" -ForegroundColor Green
        } catch {
            Write-Warning "  ⚠️  Chyba při konfiguraci služeb: $_"
        }
    } else {
        Write-Host "  ℹ️  Seznam služeb je prázdný." -ForegroundColor Gray
    }

    Write-Host ""

    # ───────────────────────────────────────────────────────────────────────
    # FÁZE 3: APPX BALÍČKY (REŽIM A - AGRESIVNÍ)
    # ───────────────────────────────────────────────────────────────────────
    Write-Host "FÁZE 3: ODSTRAŇUJI APPX BALÍČKY..." -ForegroundColor Yellow
    Write-Host ""

    # HP/OEM bloatware (úplný seznam pro AGRESIVNÍ režim)
    ...
    # Microsoft apps 
    $msApps_Aggressive = @("Microsoft.549981C3F5F10", "Microsoft.3DBuilder", "Microsoft.BingFinance", "Microsoft.BingFoodAndDrink", "Microsoft.BingHealthAndFitness", "Microsoft.BingNews", "Microsoft.BingSearch", "Microsoft.BingSports", "Microsoft.BingTranslator", "Microsoft.BingTravel", "Microsoft.BingWeather", "Microsoft.GamingApp", "Microsoft.GamingServices", "Microsoft.GetHelp", "Microsoft.Getstarted", "Microsoft.Messaging", "Microsoft.Microsoft3DViewer", "Microsoft.MicrosoftJournal", "Microsoft.MicrosoftOfficeHub", "Microsoft.MicrosoftPowerBIForWindows", "Microsoft.MicrosoftSolitaireCollection", "Microsoft.MicrosoftStickyNotes", "Microsoft.MinecraftUWP", "Microsoft.MixedReality.Portal", "Microsoft.NetworkSpeedTest", "Microsoft.News", "Microsoft.Office.Sway", "Microsoft.OneConnect", "Microsoft.OutlookForWindows", "Microsoft.People", "Microsoft.PowerAutomateDesktop", "Microsoft.Print3D", "Microsoft.RemoteDesktop", "Microsoft.ScreenSketch", "Microsoft.SkypeApp", "Microsoft.StartExperiencesApp", "Microsoft.Todos", "Microsoft.Wallet", "Microsoft.Whiteboard", "Microsoft.Windows.DevHome", "Microsoft.WindowsAlarms", "microsoft.windowscommunicationsapps", "Microsoft.WindowsFeedbackHub", "Microsoft.WindowsMaps", "Microsoft.ZuneMusic", "Microsoft.ZuneVideo", "MicrosoftCorporationII.MicrosoftFamily", "MicrosoftCorporationII.QuickAssist")

    $appsToRemove = $msApps_Aggressive + $hpOemBloatware
    ...
    Write-Host "  ✅ TWEAK A DOKONČEN!" -ForegroundColor Green
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host ""
    Write-Host "⚠    Aplikace obnov z MsStore !" -ForegroundColor Red
...
function Get-TweakARegistryList {
    [CmdletBinding()]
    [OutputType([System.Object[]])]
    param()

    if ($null -ne $script:TweakARegistryListCache) {
        return $script:TweakARegistryListCache
    }
    ...
    foreach ($rawLine in $lines) {
        ...
        if ($line -match '^\[(.+)\]$') {
            $currentPath = Convert-TweakARegistryPath -RawPath $matches[1]
            continue
        }
    ...
    $script:TweakARegistryListCache = $items.ToArray()
    return $script:TweakARegistryListCache
}

function Get-TweakASnapshot {
    [CmdletBinding()]
    [OutputType([System.Object[]])]
    param()

    $registryItems = Get-TweakARegistryList
...
Export-ModuleMember -Function @(
    'Invoke-TweakA',
    'Get-TweakAServiceList',
    'Get-TweakARegistryList',
    'Get-TweakASnapshot',
    'Invoke-ModuleEntry'
)
