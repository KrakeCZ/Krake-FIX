# ═══════════════════════════════════════════════════════════════════════════
# Module: Tweak0.psm1 - REGISTRY
# ═══════════════════════════════════════════════════════════════════════════
# REŽIM 0: ZMĚNA POUZE REGISTRŮ - BEZ ODSTRAŇOVÁNÍ APLIKACÍ/SLUŽEB/ÚLOH
# ═══════════════════════════════════════════════════════════════════════════
...
; ================================================================
; KRAKE-FIX Registry Tweaks - REŽIM REGISTRY
; ================================================================
...
$script:Tweak0RegistryListCache = $null

function Convert-Tweak0RegistryPath {
    [CmdletBinding()]
    param(
...
$script:Tweak0ServiceList = @(    
)


function Get-Tweak0ServiceList {
    [CmdletBinding()]
    [OutputType([pscustomobject[]])]
    param()

    return $script:Tweak0ServiceList
}

function Invoke-Tweak0 {
    [CmdletBinding()]
    param()

    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Magenta
    Write-Host "  TWEAK pouze pro Registry - BEZ appX/úloh/služeb" -ForegroundColor Magenta
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Magenta
    Write-Host ""
    ...
    $regContent = Get-Tweak0RegContent
    ...
    Write-Host "  ✅ TWEAK REGISTRŮ DOKONČEN!" -ForegroundColor Green
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host ""
    Write-Host "⚠️  VAROVÁNÍ: Pokud zvuk praská, nastav audio menu 17 - audio affinyty na poslední nebo všechny cpu!" -ForegroundColor Red
...
function Get-Tweak0RegistryList {
    [CmdletBinding()]
    [OutputType([System.Object[]])]
    param()

    if ($null -ne $script:Tweak0RegistryListCache) {
        return $script:Tweak0RegistryListCache
    }
    ...
    foreach ($rawLine in $lines) {
        ...
        if ($line -match '^\[(.+)\]$') {
            $currentPath = Convert-Tweak0RegistryPath -RawPath $matches[1]
            continue
        }
    ...
    $script:Tweak0RegistryListCache = $items.ToArray()
    return $script:Tweak0RegistryListCache
}

function Get-Tweak0Snapshot {
    [CmdletBinding()]
    [OutputType([System.Object[]])]
    param()

    $registryItems = Get-Tweak0RegistryList
...
Export-ModuleMember -Function @(
    'Invoke-Tweak0',
    'Get-Tweak0ServiceList',
    'Get-Tweak0RegistryList',
    'Get-Tweak0Snapshot',
    'Invoke-ModuleEntry'
)
